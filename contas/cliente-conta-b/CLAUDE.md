# CONTA: Conta B  (cliente-conta-b)

> **TRAVA OPERACIONAL — leia antes de qualquer ação.**
> Esta pasta opera **exclusivamente** a conta `cliente-conta-b`, user_id `SUBSTITUIR`.
...
- Conta: Conta B — papel: secundaria
- user_id: SUBSTITUIR
- Site: MLB (Brasil)
...
1. Rode `python cli.py checar cliente-conta-b` e confirme que o nickname devolvido
   é o desta conta.
...
```bash
python cli.py coletar cliente-conta-b      # snapshot + alertas
python cli.py alertas cliente-conta-b      # o que apareceu nas últimas 24h
